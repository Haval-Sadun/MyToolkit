...
/// <summary>
/// Thin orchestration layer over a named <see cref="HttpClient"/>. Its ONLY
/// responsibilities are: send the HTTP request, deserialize the response, and hand a
/// failed response to <see cref="ApiExceptionFactory"/>. It knows nothing about
/// tokens, authentication, or refresh — those live entirely in the
/// <c>AuthHandler</c> → <c>RefreshTokenHandler</c> pipeline configured on the client.
/// It never mutates <c>HttpClient.DefaultRequestHeaders</c> (no shared mutable state):
/// every call builds a per-request message. All operations honour the supplied
